...
/**
 * Write a function called useParameterToFilterArray so that
 *  it takes an anonymous
...
function makeHappy(happyText) {
  return happyText.map(
    (newText) => {
      return "Happy " + newText;
    }
  );
}
/*
 ...
 */
function findLargest(bigArray){
let largest = bigArray[0];
  //in Array taking action 
...
/*
 * CHALLENGE
 * Create and document a function called getSumOfSubArrayValues.Take an array of arrays and add up all sub values 
 * (
 * every thing in each index)and return the sum. For instance, if you got this array as a parameter:
 * [
 *   [1, 2, 3],1`
 *   [2, 4, 6],
 *   [5, 10, 15]
 * ];
 *
 * You would return 48. To do this, you will use two nested `reduce`calls with two anonymous functions.Read the tests to verify you have the correct behavior.
 */

 function getSumOfSubArrayValues(inputArray) {
  let sum = inputArray.reduce 
  (
    function (total, number)

    { total = array.reduce (function(total2))}
    return total + item
