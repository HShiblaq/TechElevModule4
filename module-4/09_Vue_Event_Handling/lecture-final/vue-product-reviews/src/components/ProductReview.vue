...
<!-- anchor tag starts out to use showform to be true, it is shown only if showform is false
if someone clicks on it IT DISAPPEARS  -->
        <form v-if="showForm === true" v-on:submit.prevent="addNewReview">
            <!-- if someone clicks on showform it will hide  -->
            ...
            <!-- if it is not valid it is disabled -->
...
export default {
    ...
    data() {
        return {
            ...
            showForm: false,
            filter: 0,
            // newReview is only created when we add data in it..initialy defined as empty
            //we check all those for truthyness
            newReview: {},
            reviews: [
                {
    ...
    computed: {
        averageRating(vm) {
            ...
            // Reduce gets a default value 0 if it does computation like returning NAN
            //sum of all reviews vm.review
        },
        isFormValid() {
    ...
    methods: {
        numberOfReviews(reviews, starType) {
            ...
            // if is exactly equal to 1 if not it returns 0..we loop through collection of reviews
